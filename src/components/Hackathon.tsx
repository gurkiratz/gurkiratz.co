...
interface Hackathon {
  ...
  function Hackathon({ hackathon }: { hackathon: Hackathon }) {
    let startLabel =
      typeof hackathon.start === 'string' ? hackathon.start : hackathon.start.label
    let startDate =
      typeof hackathon.start === 'string' ? hackathon.start : hackathon.start.dateTime
  
    let endLabel = typeof hackathon.end === 'string' ? hackathon.end : hackathon.end.label
    let endDate = typeof hackathon.end === 'string' ? hackathon.end : hackathon.end.dateTime
      ...
      <li className="flex gap-4">
        ...
        <dl className="flex flex-auto flex-wrap gap-x-2">
          ...
          <dd className="flex items-center gap-3 text-zinc-500 dark:text-zinc-300">
            <span className='font-semibold font-mono italic text-sm'>{hackathon.project}:</span>
            <Link className='text-md' href={hackathon.github} target='_blank'><FaGithub className='inline hover:scale-[110%] transition-all duration-150 ease-in-out' /></Link>
            {hackathon.website && <Link href={hackathon.website} target='_blank' className='text-sm hover:underline decoration-orange-600 decoration-2 '>Website <FaExternalLinkAlt className='inline' />
            </Link>}
            {hackathon.blog && <Link href={hackathon.blog} className='text-sm hover:underline decoration-orange-600 decoration-2 '>BLog</Link>}
          </dd>
